...
export default function ScanPage() {
        ...
        if (orgSnap.exists()) {
          ...
          liveOrgUnsub = onSnapshot(
            ...
            async (liveSnap) => {
              ...
              let livePlan: "free" | "pro" =
                liveData.plan === "pro" ? "pro" : "free";
  ...
  const markCheckedIn = async (guestName: string, durationMs?: number) => {
    ...
    for (const guestDoc of guestsSnap.docs) {
      ...
      if (currentCount >= maxScans) {
        setResult({
          status: "error",
          message: "Already inside (max scans reached).",
        });
        return false;
      }
  ...
  const startScan = async () => {
    ...
    if (!("BarcodeDetector" in window)) {
      setResult({
        status: "error",
        message: "Barcode detector not supported on this browser.",
      });
      return;
    }
    try {
      ...
      if (capabilities && "zoom" in capabilities) {
        ...
        try {
          await track.applyConstraints({
            advanced: [{ zoom: Math.min(zoomCaps.max, 2) }],
      ...
      const scanLoop = async () => {
        ...
        if (barcodes.length > 0) {
          ...
          const durationMs = scanStart
            ? Math.max(0, Math.round(performance.now() - scanStart))
            : undefined;
      ...
      <div className="max-w-[900px] mx-auto px-6 sm:px-10 py-12">
        ...
        >
          ...
          <p className="text-slate-600 mt-2">
            Scan guest QR codes to check them in live.
          </p>
        </motion.div>
          ...
          <div className="border border-slate-200 rounded-3xl p-6 bg-white shadow-sm space-y-4">
            ...
            {isFree ? (
              <div className="text-xs text-slate-500">
                QR check-in is disabled in free mode.
              </div>
            ) : null}
            {result.message ? (
              <div
        ...
        {blockedOrgOpen ? (
          ...
          >
            ...
            >
              ...
              <p className="text-sm text-slate-600 mb-4">
                This organization has been blocked by an administrator. You will
                be redirected.
              </p>
              <button
                type="button"
