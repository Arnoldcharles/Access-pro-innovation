...
export default function OrgDeletedPage() {
  ...
  const handlePermanentDelete = async () => {
    ...
    const eventsSnap = await getDocs(
      collection(db, "orgs", deletedSlug, "events"),
    );
    for (const eventDoc of eventsSnap.docs) {
      const eventId = eventDoc.id;
      const guestsSnap = await getDocs(
        collection(db, "orgs", deletedSlug, "events", eventId, "guests"),
      );
      for (const guest of guestsSnap.docs) {
        await deleteDoc(guest.ref);
      }
      const invitesSnap = await getDocs(
        collection(db, "orgs", deletedSlug, "events", eventId, "invites"),
      );
    ...
    if (currentUserId) {
      ...
      const currentCount = userSnap.exists()
        ? ((userSnap.data().orgCount as number) ?? 0)
        : 0;
      await updateDoc(doc(db, "users", currentUserId), {
        orgCount: Math.max(0, currentCount - 1),
      });
    }
    const orgsQuery = query(
      collection(db, "orgs"),
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className="font-semibold">
                    {deletedSlug || "Deleted organization"}
                  </div>
